...
import { AllocationTeamAllocation, ProjectAllocation } from '@/types/business';

interface TeamAllocationEditModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  projectId: string;
  allocation: ProjectAllocation;
  teamAllocation: AllocationTeamAllocation;
}

export const TeamAllocationEditModal: React.FC<TeamAllocationEditModalProps> = ({
  open,
  onOpenChange,
  projectId,
  allocation,
  ...
  const [allocationType, setAllocationType] = useState<'percentage' | 'fixed'>(teamAllocation.allocationType);
  const [allocationValue, setAllocationValue] = useState(teamAllocation.allocationValue.toString());
  ...
  const member = data.teamMembers.find(m => m.id === teamAllocation.memberId);
  ...
  const budgetTotals = useMemo(() => {
    ...
    const teamTotal = allocationTeamAllocations
      .filter(a => a.memberId !== teamAllocation.memberId) // Exclude current allocation
  ...
  }, [allocation, project, teamAllocation.memberId]);
  ...
  const paidAmount = useMemo(() => {
    return data.payments
      .filter(payment => 
        ...
        payment.memberId === teamAllocation.memberId && 
        payment.allocationId === allocation.id &&
        payment.status === 'completed'
      )
      .reduce((sum, payment) => sum + payment.amount, 0);
  }, [data.payments, projectId, teamAllocation.memberId, allocation.id]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      dispatch({
        type: 'UPDATE_ALLOCATION_TEAM_ALLOCATION',
        payload: {
          projectId,
          allocationId: allocation.id,
          memberId: teamAllocation.memberId,
          updates: {
            allocationType,
